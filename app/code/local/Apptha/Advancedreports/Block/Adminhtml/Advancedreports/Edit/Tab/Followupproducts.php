...
?>
<?php
class Apptha_Advancedreports_Block_Adminhtml_Advancedreports_Edit_Tab_Followupproducts extends Mage_Adminhtml_Block_Widget_Form {

    // Assigning followupproducts report template
    protected function _construct() {
        parent::_construct();
        $this->setTemplate('advancedreports/followupproducts.phtml');
    }   
           
    // Geting products collection   
    public function getProductDetailsCollection($products,$db_from,$db_to) {
       
        
        if(!empty($products))
        {          
        $orderTotals = Mage::getModel('sales/order_item')->getCollection()
                       ->addAttributeToFilter('main_table.product_id',$products); 
