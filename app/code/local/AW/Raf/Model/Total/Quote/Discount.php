...
class AW_Raf_Model_Total_Quote_Discount extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if ($helper->getCustomerId() && ($address->getBaseGrandTotal() > 0)) {
            ...
            if (Mage::helper('tax')->applyTaxAfterDiscount(Mage::app()->getStore())) {
    ...
    protected function _splitAmount($address)
    {
        ...
        foreach ($address->getQuote()->getItemsCollection() as $item) {
            if(
                ...
                && $item->getProduct()->getPriceType() == Mage_Bundle_Model_Product_Price::PRICE_TYPE_DYNAMIC
            ) {
                continue;
            }
    ...
    protected function _calculateTax($address)
    {
        $helper = Mage::helper('awraf');
        $taxConfig = Mage::getSingleton('tax/config');
        $store = Mage::app()->getStore();
        $calculator = Mage::getSingleton('tax/calculation');
        $inclTax = $taxConfig->priceIncludesTax($store);
        $rafFetch = $helper->getAppliedDiscount() + $helper->getAppliedAmount();
        $calculator->setCustomer($address->getQuote()->getCustomer());

        $rates = array();
        $totalTax = $address->getTotalAmount('tax');
        $totalBaseTax = $address->getBaseTotalAmount('tax');
        foreach ($address->getAppliedTaxes() as $rate) {
            $awRafBaseTax = $calculator->calcTaxAmount($rafFetch, $rate['percent'], $inclTax, true);
            $awRafTax = $store->convertPrice($awRafBaseTax);
            $rate['amount'] = $rate['amount'] - $awRafTax;
            $rate['base_amount'] = $rate['base_amount'] - $awRafBaseTax;
            $rates[] = $rate;
            $totalTax -= $awRafTax;
            $totalBaseTax -= $awRafBaseTax;
        }

        $address->setAppliedTaxes($rates);
        $address->setTotalAmount('tax', $totalTax);
        $address->setBaseTotalAmount('tax', $totalBaseTax);
    }
