 ...
 * @author      Magespacex Developer
 */
class Magespacex_Customercredit_Model_Observer
{
     ...
     *
     * @return Magespacex_Customercredit_Model_Observer
     */
    ...
    public function customercreditPaymentMethod($observer)
    {
        ...
        if ($block instanceof Mage_Checkout_Block_Onepage_Payment_Methods || $block instanceof Magespacex_Webpos_Block_Onepage_Payment_Methods) {
        ...
        if (($block instanceof Magespacex_RewardPoints_Block_Checkout_Cart_Rewrite_Coupon) || ($block instanceof Mage_Checkout_Block_Cart_Coupon)) {
            $requestPath = $block->getRequest()->getRequestedRouteName()
                . '_' . $block->getRequest()->getRequestedControllerName()
                . '_' . $block->getRequest()->getRequestedActionName();
    ...
    public function orderPlaceAfter($observer)
    {
        ...
        if ($amount && !$session->getHasCustomerCreditItem()) {
            Mage::getModel('customercredit/transaction')->addTransactionHistory($customer_id, Magespacex_Customercredit_Model_TransactionType::TYPE_CHECK_OUT_BY_CREDIT, Mage::helper('customercredit')->__('check out by credit for order #') . $order->getIncrementId(), $order->getId(), -$amount);
...
//            $total_customercredit_invoiced = $data['total_customercredit_invoiced'];
            ...
            $type_id = Magespacex_Customercredit_Model_TransactionType::TYPE_CANCEL_ORDER;
    ...
    public function invoiceSaveAfter($observer)
    {
            ...
            if (!$item->getParentItemId()) {
                ...
                if ($type == 'customercredit') {
                    if (isset($buyRequest['send_friend']) && isset($buyRequest['recipient_email']) && $buyRequest['send_friend']
                        ...
                        Mage::getModel('customercredit/transaction')->addTransactionHistory($customer_id, Magespacex_Customercredit_Model_TransactionType::TYPE_SHARE_CREDIT_TO_FRIENDS, $customer_email . " sent " . $amount . " credit to " . $email, "", 0);
        ...
        if ($product_credit_value > 0) {
            Mage::getModel('customercredit/transaction')
                ->addTransactionHistory($order->getCustomerId(), Magespacex_Customercredit_Model_TransactionType::TYPE_BUY_CREDIT, "buy credit " . $product_credit_value . " from store ", $order->getId(), $product_credit_value);
    ...
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        $type_id = Magespacex_Customercredit_Model_TransactionType::TYPE_REFUND_ORDER_INTO_CREDIT;
        ...
        if ($product_credit_value > 0) {
            $type_id = Magespacex_Customercredit_Model_TransactionType::TYPE_REFUND_CREDIT_PRODUCT;
