 ...
 * @author      Magespacex Developer
 */
class Magespacex_Customercredit_Model_Total_Quote_Discountbeforetax extends Mage_Sales_Model_Quote_Address_Total_Abstract {

    protected $_hiddentBaseDiscount = 0;
    protected $_hiddentDiscount = 0;

    /**
     * Magespacex_Customercredit_Model_Total_Quote_Discountbeforetax constructor.
     */
...
//        }
        ...
        foreach ($address->getAllItems() as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if (!$child->isDeleted() && $child->getProduct()->getTypeId() != 'customercredit') {
                        if (Mage::helper('tax')->priceIncludesTax())
                            $itemDiscount = $child->getRowTotalInclTax() - $child->getBaseDiscountAmount() - $child->getMagespacexBaseDiscount();
                        else
                            $itemDiscount = $child->getBaseRowTotal() - $child->getBaseDiscountAmount() -$child->getMagespacexBaseDiscount();
                ...
                if (!$item->isDeleted() && $item->getProduct()->getTypeId() != 'customercredit') {
                    if (Mage::helper('tax')->priceIncludesTax())
                        $itemDiscount = $item->getRowTotalInclTax() - $item->getBaseDiscountAmount() - $item->getMagespacexBaseDiscount();
                    else
                        $itemDiscount = $item->getBaseRowTotal() - $item->getBaseDiscountAmount() - $item->getMagespacexBaseDiscount();
                    $baseDiscountTotal += $itemDiscount;
                }
            }
        }
        ...
        if ($helper->getSpendConfig('shipping')) {
            if (Mage::helper('tax')->shippingPriceIncludesTax())
                $shippingDiscount = $address->getShippingInclTax() - $address->getBaseShippingDiscountAmount() - $address->getMagespacexBaseDiscountForShipping();
            else
                $shippingDiscount = $address->getBaseShippingAmount() - $address->getBaseShippingDiscountAmount() - $address->getMagespacexBaseDiscountForShipping();
...
//        $address->setMagespacexBaseDiscount($address->getMagespacexBaseDiscount() + $baseTotalDiscount);
    ...
    public function _prepareDiscountCreditForAmount(Mage_Sales_Model_Quote_Address $address, $rate, $baseCustomercreditForShipping) {
        ...
        foreach ($address->getAllItems() as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if(!$child->isDeleted() && $child->getProduct()->getTypeId() != 'customercredit') {
                        if (Mage::helper('tax')->priceIncludesTax())
                            $baseItemPrice = $child->getRowTotalInclTax() - $child->getBaseDiscountAmount() - $child->getMagespacexBaseDiscount();
                        else
                            $baseItemPrice = $child->getBaseRowTotal() - $child->getBaseDiscountAmount() - $child->getMagespacexBaseDiscount();
                ...
                if(!$item->isDeleted() && $item->getProduct()->getTypeId() != 'customercredit') {
                    if (Mage::helper('tax')->priceIncludesTax())
                        $baseItemPrice = $item->getRowTotalInclTax() - $item->getBaseDiscountAmount() - $item->getMagespacexBaseDiscount();
                    else
                        $baseItemPrice = $item->getBaseRowTotal() - $item->getBaseDiscountAmount() - $item->getMagespacexBaseDiscount();
                    $itemBaseDiscount = $baseItemPrice * $rate;
                    $itemDiscount = Mage::app()->getStore()->convertPrice($itemBaseDiscount);
        ...
        if ($helper->getSpendConfig('shipping') && $baseCustomercreditForShipping) {
            if (Mage::helper('tax')->shippingPriceIncludesTax())
                $baseShippingPrice = $address->getShippingInclTax() - $address->getBaseShippingDiscountAmount() - $address->getMagespacexBaseDiscountForShipping();
            else
                $baseShippingPrice = $address->getBaseShippingAmount() - $address->getBaseShippingDiscountAmount() - $address->getMagespacexBaseDiscountForShipping();
