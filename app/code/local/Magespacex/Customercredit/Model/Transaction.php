 ...
 * @author      Magespacex Developer
 */
class Magespacex_Customercredit_Model_Transaction extends Mage_Core_Model_Abstract
{
    ...
    public function addTransactionHistory($customer_id, $transaction_type_id, $transaction_detail, $order_id, $amount_credit)
    {
        ...
        $spent_credit = 0;
        $received_credit = 0;
        if ($transaction_type_id == Magespacex_Customercredit_Model_TransactionType::TYPE_CHECK_OUT_BY_CREDIT) {
            $spent_credit = ($amount_credit > 0) ? $amount_credit : -$amount_credit;
        } elseif ($transaction_type_id == Magespacex_Customercredit_Model_TransactionType::TYPE_REFUND_ORDER_INTO_CREDIT) {
            $received_credit = ($amount_credit > 0) ? $amount_credit : -$amount_credit;
        }

        if ($transaction_type_id == Magespacex_Customercredit_Model_TransactionType::TYPE_BUY_CREDIT) {
            $received_credit = ($amount_credit > 0) ? $amount_credit : -$amount_credit;
        }
