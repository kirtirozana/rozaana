 ...
 * @author      Magespacex Developer
 */
class Magespacex_Customercredit_IndexController extends Mage_Core_Controller_Front_Action
{
        ...
        $credit = Mage::getModel('customercredit/creditcode')->getCollection()
         ...
         if ($credit->getSize() == 0 || $credit->getFirstItem()->getStatus() == 4) {
            ...
            Mage::getModel('customercredit/creditcode')
                ->changeCodeStatus($credit->getFirstItem()->getId(), Magespacex_Customercredit_Model_Status::STATUS_USED);
            $credit_amount = $credit->getFirstItem()->getAmountCredit();
            Mage::getModel('customercredit/transaction')->addTransactionHistory($customer_id, Magespacex_Customercredit_Model_TransactionType::TYPE_REDEEM_CREDIT, "redeem credit by code '" . $credit_code . "'", "", $credit_amount);
...
//            return $this->_redirect('customer/account/login');
    ...
    public function cancelAction()
    {
        ...
        Mage::getModel('customercredit/transaction')->addTransactionHistory($customer_id, Magespacex_Customercredit_Model_TransactionType::TYPE_CANCEL_SHARE_CREDIT, "cancel share credit ", "", $add_balance);
        Mage::getModel('customercredit/customercredit')->changeCustomerCredit($add_balance);
        Mage::getModel('customercredit/creditcode')->changeCodeStatus($credit_code_id, Magespacex_Customercredit_Model_Status::STATUS_CANCELLED);
        return $this->_redirect('*/index/share');
    }
    ...
    public function sharepostAction()
    {
        ...
        if (Mage::helper('customercredit')->getGeneralConfig('validate')) {
            ...
            if (trim($keycode) == trim(Mage::getSingleton('core/session')->getData("emailcode"))) {
                ...
                Mage::getModel('customercredit/transaction')->addTransactionHistory($customer_id, Magespacex_Customercredit_Model_TransactionType::TYPE_SHARE_CREDIT_TO_FRIENDS, $customer_email . " sent " . $amount . " credit to " . $email, "", -$amount);
                ...
                if (isset($friend_account_id)) {
                    Mage::getModel('customercredit/transaction')
                        ->addTransactionHistory($friend_account_id, Magespacex_Customercredit_Model_TransactionType::TYPE_RECEIVE_CREDIT_FROM_FRIENDS, $email . " received " . $amount . " credit from " . $customer_name, "", $amount);
                    Mage::getModel('customercredit/customercredit')->addCreditToFriend($amount, $friend_account_id);
                } else {
                    if (isset($credit_code_id)) {
                        Mage::getModel('customercredit/creditcode')->changeCodeStatus($credit_code_id, Magespacex_Customercredit_Model_Status::STATUS_UNUSED);
...
//                Mage::getModel('customercredit/customercredit')->sendCodeToFriendEmail($email, $amount, $message);
    ...
    public function validateCustomerAction()
    {
        ...
        if ($recipient_email && $credit_amount && !isset($customer_id) && ($is_send_email == 'yes')) {
            $credit_code = Mage::getModel('customercredit/creditcode')->addCreditCode($recipient_email, $credit_amount, Magespacex_Customercredit_Model_Status::STATUS_AWAITING_VERIFICATION, $sender_id);
    ...
    public function checkCreditAction()
    {
        ...
        if (isset($modulesArray['Magespacex_Onestepcheckout']) && Mage::getStoreConfig('onestepcheckout/general/active') == '1') {
