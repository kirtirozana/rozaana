 ...
 */
class Textlocal_SMSNotifications_Model_Textlocal
{
	...
	private function _sendRequest($command, $params = array())
	{
		...
		// $apikey= $settings['sms_auth_token'];
		//$this->apiKey='BHjpSxtFrB4-jy8lstnIU5BL93qfl3oAjX2x7IVbFe';
		$this->apiKey =  $settings['sms_auth_token'];
		...
		$result = json_decode($rawResponse);
          ...
          if($result->status == 'failure'){
				/*Mage::getSingleton('adminhtml/session')->addError('Invalid Api Key.');*/
			return;
			}

			if($result->status == 'success'){
				//Mage::getSingleton('adminhtml/session')->addSuccess(sprintf('The setting saved successfully'));
			}
